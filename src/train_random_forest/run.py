...
def go(args):
    ...
    ############################################################
    # 🔥 NEW: Flatten RF config so W&B shows each parameter
    ############################################################
    for k, v in rf_config.items():
        run.config[f"rf__{k}"] = v

    # Log TFIDF feature count as a column
    run.config["max_tfidf_features"] = args.max_tfidf_features
    ############################################################
